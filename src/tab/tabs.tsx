...
export class Tabs extends React.Component<TabsProps, TabsState> {
  ...
  private tabListRef = React.createRef<HTMLDivElement>();
  ...
  constructor(props: React.PropsWithChildren<TabsProps>) {
    super(props);
    ...
    const wrap = (
      <div className={clsx(bem('wrap', { scrollable }), { [BORDER_TOP_BOTTOM]: type === 'line' && bordered })}>
        <div
          ref={this.tabListRef}
          role="tablist"
          className={bem('nav', [type, { complete: !ellipsis }])}
          style={navStyle}
        >
          {navLeft}
          {navs}
          {type === 'line' && <div ref={this.lineRef} className={bem('line')} style={lineStyle} />}
          {navRight}
        </div>
      </div>
    );
  ...
  private genContents(): JSX.Element {
    const props = this.props;
    ...
    const wrap = (
      <div ref={this.contentRef} className={bem('content', { animated: animated })}>
