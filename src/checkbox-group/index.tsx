...
export class CheckboxGroup extends React.Component<CheckboxGroupProps, CheckboxGroupState> {
  ...
  private genOption(option: CheckboxGroupOption, passProps: CheckboxProps): JSX.Element {
    const props = this.props;
    ...
    return (
      <Checkbox key={option.value} {...checkboxProps} onClick={this.bindedOnClick}>
        {label}
      </Checkbox>
    );
  }

  private genOptions(): JSX.Element | JSX.Element[] {
    const props = this.props;
    const { options } = props;
  ...
  render(): JSX.Element {
    return <div className={bem([this.props.direction])}>{this.genOptions()}</div>;
  }
}
